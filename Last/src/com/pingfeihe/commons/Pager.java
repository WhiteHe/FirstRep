...
public class Pager {
	...
	private void process() {
		...
		this.prev = this.pageNum == this.first ? 1 : this.pageNum - 1;
		this.next = this.pageNum == this.last ? this.last : this.pageNum + 1;
		if (this.pageCount >= this.navPageSize) {
			if ((this.pageNum - ((int) Math.ceil(this.navPageSize * 1.0 / 2))) <= 0) {
				this.pageStart = 1;
				this.pageEnd = this.navPageSize;
			} else {
				if ((this.pageCount - this.pageNum) < ((int) Math
						.ceil(this.navPageSize * 1.0 / 2))) {
					this.pageStart = this.pageCount - this.navPageSize + 1;
					this.pageEnd = this.pageCount;
				} else {
					this.pageStart = this.pageNum
							- ((int) Math.ceil(this.navPageSize * 1.0 / 2)) + 1;
					this.pageEnd = this.pageNum
							+ ((int) Math.ceil(this.navPageSize * 1.0 / 2)) - 1;
				}
			}
		} else {
